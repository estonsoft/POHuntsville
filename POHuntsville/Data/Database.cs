...
namespace POHuntsville
{
    public class Database
    {
        ...
        public async Task<List<Item>> SearchItems(String sSearch, Category category, String sBarcode, Subcategory subcategory, Subsubcategory subsubcategory)
        {
            ...
            if (sBarcode.Length <= 6 && decimal.TryParse(sBarcode, out dItemNo))
            {
            ...
            if (!string.IsNullOrEmpty(sBarcode))
            {
                ...
                (([UPC_3] like '%{sBarcode}%' or [UPC_3] like '%{sBarcodeShort}%') and [UPC_3] > '') OR
                (([UPC_4] like '%{sBarcode}%' or [UPC_4] like '%{sBarcodeShort}%') and [UPC_4] > '')) OR
        ...
        {
            Decimal dItemNo = 0;
            if (sSearch.Length <= 6 && decimal.TryParse(sSearch, out dItemNo))
            {
            }
